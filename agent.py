...
# --- SYSTEM PROMPT ---
BASE_SYSTEM_PROMPT = """You are a helpful robot assistant running on an Orange Pi.
    ...
    def chat_and_respond(self, text, img_path=None):
        ...
        try:
            ...
            for chunk in self._llm_chat_stream(text):
                ...
                # Detect JSON action response
                if ('{"' in chunk or '"action"' in chunk) and not is_action_mode:
                    is_action_mode = True
                    self.thinking_sound_active.clear()
                    continue
